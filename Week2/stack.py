...
def valid(test_string):
	...
	for each in test_string:
		if each in ["(","["]:
			validate_stack.append(each)
		elif each in [")","]"]:
			try:
				last_pop = validate_stack.pop()
			except:
				# .pop() will fail if stack is empty. Should not be empty if a closing ) or ] exist.
				return False
			else:
				if (last_pop == "(" and each == ")") or (last_pop == "[" and each == "]"):
					pass
				else:
					return False
	else:
